(function () {
  ...
  var touch = function touch(vv) {
    if (evaluator.isUpdating()) {
      WARNING("ignoring touch during evaluation;" +
        " assumed to be direct output: " + vv);
      return;
    }
  ...
  var enqueue = function enqueue(mm) {
    if (evaluator.isUpdating()) {
      WARNING("cannot enqueue during evaluation: " + mm);
      return;
    }
  ...
  var update = function update() {
    ...
    touchedSet.setUnion(changedSet).forEach(function (vv) {
      vv.publishAll();
    });
  };
