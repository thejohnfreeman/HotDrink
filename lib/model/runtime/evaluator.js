(function () {
  ...
  var maybeExecute = function maybeExecute(mm) {
    ...
    if (!mm.needsExecution) {
      ...
      mm.needsExecution = mm.inputsUsed.some(function (uu) {
        maybeSet(uu);
        ASSERT(uu.lastUpdated > timestamp,
          "maybeExecute: input should be updated");
  ...
  var execute = function execute(mm) {
    ...
    var results = (mm.fn) ? (mm.fn.call(mm.context)) : (undefined);
    executedSet.push(mm);
  ...
  var set = function set(vv) {
    ...
    if (!vv.isChanged()) {
       ...
       * by `maybeSet`. */
      LOG("set: " + (vv.isSource() ? "source" : "output") + " variable " +
        vv + " was not changed");
      return;
    }
