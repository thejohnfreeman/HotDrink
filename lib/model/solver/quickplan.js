(function () {
           ...
           && this.freeVariableSet.length > 0)
    {
      ...
      if (vvvFree.isFree()) {
        ...
        ccc.outer.methods.forEach(function (mm) {
          ...
          if (mm.outputs.length >= mmNumOutputs) { 
            return;
          }
    ...
    = function eliminateConstraint(ccc, mmNext)
  {
    ...
    var newlyUndeterminedVars = outputsPrev.setSubtract(outputsNext);
    ...
    newlyUndeterminedVars.forEach(function (vv) {
       ...
       * We do not want to put free variables into the potentially
       * undetermined variables queue, so we check that v.numConstraints 
       * is greater than 2.
       */
     ...
     * necessary. */
    this.undoStack.push([ccc, mmPrev]);

    this.changeSelectedMethod(ccc, mmNext);
    ...
    = function collectUpstreamConstraints(ccc)
  {
    ...
    ccc.variables.forEach(function (vvv) {
      ...
      if (cccUp && cccUp.mark !== this.mark.upstream) {
        LOG("CUC: moving upstream from " + vvv);
        INSPECT(vvv);
  ...
  /**
   * Change the selectedMethod for a constraint.
   * @param {Constraint} ccc
   * @param {Method} mmNew
    ...
    = function changeSelectedMethod(ccc, mmNew)
  {
    ...
    if (mmNew) {
      ...
      mmNew.outputs.forEach(function (ww) {
        ...
        /* As an optimization, only assign writtenBy if mmNew is not a
         * stay-method. */
        if (mmNew.fn) {
          ww.writtenBy = mmNew;
        }
  ...
  Solver.prototype.undo = function undo() {
    while (this.undoStack.length > 0) {
      ...
      this.changeSelectedMethod(ccc, mmPrev);
